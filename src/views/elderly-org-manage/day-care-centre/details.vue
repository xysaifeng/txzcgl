...
<template>
  <el-container>
    ...
    <el-main class="el-main-custom nopadding-custom">
      ...
      <!-- 机构运营 -->
      <org-operation ref="orgOperation" v-show="tabsPanelCustomActiveValue === 5"></org-operation>
      <!-- 人员信息 -->
      <personal-information ref="personalInformation" v-show="tabsPanelCustomActiveValue === 6"></personal-information>
      <el-button type="primary" @click="handelClearLocalStore">清除localStore</el-button>
...
export default {
  ...
  mounted() {
    ...
    let _getForm_ = JSON.parse(localStorage.getItem('day-care-centre'))
    if(_getForm_ !== null){ this.echoAllChildrenForm(_getForm_) }
  ...
  methods: {
    ...
    // 上一步
    handelClickPreStep() {
      // ================模拟接口，将数据暂存到localStore中=====================
      let _form_ = this.getAllChildrenForm()
      localStorage.setItem('day-care-centre',JSON.stringify(_form_))
      ...
      if (this.modelType === 'look') {
        ...
        switch(this.tabsPanelCustomActiveValue){
          ...
          case 4:
            ...
            break;
          case 5:
            thisValidateForm = this.$refs.orgOperation.childrenValidateForm()
            break;
          case 6:
        ...
        // 当前面板校验通过，成功进入下一步骤
        if(thisValidateForm){
          if(this.tabsPanelCustomActiveValue === 6){
            // 到第6步，验证通过后，则提交表单
            this.closeDetailsPage()
            this.$message({
              type: 'success',
    ...
    echoAllChildrenForm(allForm){
      ...
      this.$refs.orgOperation.echoAllChildrenForm(allForm)
    ...
    getAllChildrenForm(){
      ...
      let form5 = this.$refs.orgOperation.getAllChildrenForm()
      let form6 = this.$refs.personalInformation.getAllChildrenForm()
