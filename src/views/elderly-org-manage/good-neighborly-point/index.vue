...
// 引入接口方法
import { loadTableData } from '@/api/good-neighborly-point/index'
import areaStreetOptions from '@/assets/js/shanghai.js'
import Pagination from "@/components/Pagination"
export default {
  name: 'GoodNeighborlyPoint',
  components: {
    Pagination
  },
  data() {
  ...
  methods: {
    ...
    handleClickAdd() {
      this.$router.push({
        path: '/good-neighborly-point/details/0',
        query: {
          'id': '',
          'modelType': 'add',
          'customTitle': `新增睦邻点`
    ...
    handleClickLook(row) {
      this.$router.push({
        path: `/good-neighborly-point/details/${row.id}`,
        query: {
          'id': row.id,
          'modelType': 'look',
