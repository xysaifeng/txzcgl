      ...
      <sticky :z-index="5" :sticky-top="100" class="sticky-fixed-custom">
        <div class="header-btn-wrap-custom">
          ...
          <div class="header-btn-right-custom">
            ...
            <el-button type="primary" class="reset-btn-custom" plain v-show="tabsPanelCustomActiveValue !== 6" :disabled="btnStatus.nextStepDisabled || tabsPanelCustomActiveValue === 6" @click="handelClickNextStep">下一步</el-button>
            <el-button type="primary" v-show="this.modelType !== 'look' && tabsPanelCustomActiveValue === 6" :disabled="btnStatus.submitDisabled" @click="handelSubmitBtn">提交</el-button>
          </div>
        </div>
      </sticky>
    </el-header>
...
export default {
  name: 'GoodNeighborlyPointDetails',
  components: {
    TabsPanelCustom,
    BasicInformation,
    ServiceAbility,
  ...
  methods: {
    ...
    closeDetailsPage() {
      ...
      this.$router.push({path: '/good-neighborly-point/index'})
    },
    // 返回
    handelClickGoBack() {
      let action = ''
